...
**Glide 的解决方案**：增加 ActiveResources 层 + **引用计数**

核心不在于"是否同一个对象"，而在于 `EngineResource` 的引用计数机制：
...
class EngineResource<Z> implements Resource<Z> {
    ...
    void release() {
        if (--acquired == 0) {
            // 引用归零 → 从 ActiveResources 移除，放回 MemoryCache
...
```
场景：ImageView A 和 ImageView B 同时显示同一张图

1. A 请求图片 → 网络加载 → 放入 ActiveResources，acquired = 1
2. B 请求同一张图 → 命中 ActiveResources → acquired = 2（同一对象，计数+1）
3. A 所在页面销毁 → release() → acquired = 1（资源仍在 ActiveResources）
4. B 所在页面销毁 → release() → acquired = 0 → 移到 MemoryCache
```

如果没有 ActiveResources，只用 LruCache（MemoryCache）：

```
1. A 请求图片 → 放入 LruCache
2. B 请求同一张图 → 从 LruCache 获取
3. 内存紧张 → LruCache 淘汰该条目 → Bitmap 被 recycle()
4. A 的 ImageView 还在显示这个 Bitmap → 崩溃或显示异常
```

> ActiveResources 的本质作用：**把正在被引用的资源"保护"起来，不受 LruCache 淘汰策略影响**。WeakReference 只是防泄漏的兜底——只要有强引用（ImageView 持有），资源不会被 GC；所有强引用释放后，WeakReference 被回收，资源安全回到 MemoryCache。

```
正在显示的图片 → ActiveResources（WeakReference + 引用计数保护）
不再显示但可能复用 → MemoryCache（LruCache，可被淘汰）
需要持久化 → DiskCache
```
...
**核心认知：`DiskCacheStrategy` 只控制磁盘缓存，不影响内存缓存。** 内存缓存由 `skipMemoryCache()` 单独控制，两者是独立的维度。

```java
Glide.with(context)
    .load(url)
    .diskCacheStrategy(DiskCacheStrategy.ALL)      // 控制磁盘层
    .skipMemoryCache(false)                         // 控制内存层（默认 false）
    .into(imageView);
```

**五种磁盘缓存策略详解：**

| 策略 | Data Cache（原图） | Resource Cache（转换后） | 适用场景 |
|------|:-----------------:|:----------------------:|---------|
| `AUTOMATIC` | 远程资源缓存 | 本地资源缓存 | **默认策略**，智能判断数据源 |
| `ALL` | 是 | 是 | 同一 URL 需要多种尺寸/转换 |
| `DATA` | 是 | 否 | 只缓存原图，节省磁盘空间 |
| `RESOURCE` | 否 | 是 | 原图太大不想存，只存转换结果 |
| `NONE` | 否 | 否 | 图片频繁变化（如验证码） |

> `AUTOMATIC` 的智能逻辑：对于远程资源（网络 URL），缓存原始数据（Data Cache），因为网络请求代价大；对于本地资源（File/Uri），缓存转换后结果（Resource Cache），因为本地读取快但转换有开销。

**各缓存层的控制关系总览：**

| 缓存层 | 控制方式 | 默认行为 |
|--------|---------|---------|
| ActiveResources | 自动管理，无需配置 | 始终生效，引用计数归零后移到 MemoryCache |
| MemoryCache | `skipMemoryCache(true/false)` | 默认开启 |
| DiskCache - Data | `diskCacheStrategy(...)` | AUTOMATIC：远程资源缓存 |
| DiskCache - Resource | `diskCacheStrategy(...)` | AUTOMATIC：本地资源缓存 |

### 6.2 缓存 Key 体系

**内存缓存和磁盘缓存使用不同的 Key 策略**，这是理解 Glide 缓存行为的关键。

#### 内存缓存 Key（EngineKey）

ActiveResources 和 MemoryCache 共用 `EngineKey`，包含完整的请求参数：
...
// 任一因素变化 → 不同的 Key → 同一张图不同尺寸/转换各自缓存
```

> 内存缓存是**精确匹配**：同一 URL 但不同尺寸（200x200 vs 400x400）会命中不同的缓存条目。

#### 磁盘缓存 Key（两种）

磁盘缓存有两种 Key，对应两种缓存类型：

```java
// 1. Data Cache Key（原始数据缓存）
//    只包含：model(url) + signature
//    不包含尺寸、转换等参数
//    → 同一 URL 的原图只存一份，无论请求什么尺寸

// 2. Resource Cache Key（转换后结果缓存）
//    包含：model + signature + width + height + transformations + ...
//    → 与内存缓存 Key 类似，精确匹配
```

这个双 Key 设计使得**磁盘层面的原图缓存只取决于 URL 和 signature**，与尺寸/转换无关：

```
场景：先加载 200x200 裁剪版，再加载 400x400 版本（DiskCacheStrategy.ALL）

第一次请求 200x200:
  内存未命中 → 磁盘未命中 → 网络加载
  → 写入 Data Cache（原图，Key = url）
  → 解码转换为 200x200
  → 写入 Resource Cache（Key = url + 200x200 + transform）
  → 放入 ActiveResources

第二次请求 400x400:
  内存未命中（EngineKey 不同）
  → Resource Cache 未命中（尺寸不同）
  → Data Cache 命中（Key 只看 url，命中原图）
  → 从原图重新解码转换为 400x400
  → 写入 Resource Cache（Key = url + 400x400 + transform）
  → 放入 ActiveResources
  （省去了网络请求）
```
...
### Q1：Glide 的三级缓存是什么？为什么需要 ActiveResources？

**答**：

**三级缓存**：
1. **ActiveResources**：存储正在使用的资源，使用 WeakReference + 引用计数
2. **MemoryCache**：存储近期使用的资源，使用 LruCache
3. **DiskCache**：持久化存储，使用 LruDiskCache（分 Data Cache 和 Resource Cache）

**为什么需要 ActiveResources**：

只用 LruCache 的问题：图片正在被 ImageView 显示时，如果内存紧张触发 LruCache 淘汰，Bitmap 被 recycle()，正在显示的 ImageView 会崩溃或显示异常。

ActiveResources 通过两个机制解决：
- **引用计数（acquired）**：每个使用者 acquire +1、release -1，归零后才移到 MemoryCache。多个 ImageView 显示同一张图时，只要还有一个在用，资源就不会离开 ActiveResources。
- **WeakReference**：防止泄漏。如果所有 ImageView 都销毁了但忘记 release，WeakReference 会被 GC 回收，通过 ReferenceQueue 监听后将资源安全放回 MemoryCache。

本质：**把正在被引用的资源保护起来，不受 LruCache 淘汰策略影响**。
...
### Q4：Glide 的缓存 Key 由哪些因素决定？内存和磁盘的 Key 一样吗？

**答**：

**不一样。** 内存缓存和磁盘缓存使用不同的 Key 策略：

**内存缓存 Key（EngineKey）**——精确匹配：
```java
EngineKey = f(model, signature, width, height, transformations, resourceClass, transcodeClass, options)
```
任一因素变化都会生成不同 Key，同一 URL 不同尺寸各自独立缓存。ActiveResources 和 MemoryCache 共用这套 Key。

**磁盘缓存 Key**——分两种：
- **Data Cache Key**：只包含 `model(url) + signature`，不含尺寸/转换。同一 URL 的原图只存一份。
- **Resource Cache Key**：包含完整参数（类似 EngineKey），精确匹配转换后的结果。

这个双 Key 设计的好处：第一次加载 200x200，第二次加载 400x400，虽然 Resource Cache 没命中（尺寸不同），但 Data Cache 能命中原图，省去网络请求，只需本地重新解码转换。这也是 `DiskCacheStrategy.ALL` 同时缓存原图和转换结果的意义。
