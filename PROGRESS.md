...
| SDK 开发与发布 | `Android/工程化与DevOps/SDK开发与发布.md` | **SDK 与 App 开发核心差异、三大纪律**、API 设计原则（最小暴露面/internal/@RestrictTo/api-impl 分层）、**向后兼容与二进制兼容性（三层次对比、高危操作对照表、Metalava/japicmp）**、@Deprecated 三阶段过渡、**初始化设计三方式对比与双模式支持、隐私合规两阶段模式**、依赖管理（传递依赖最小化、版本兼容策略）、**AAR 打包机制（内部结构、资源合并、consumerProguardFiles）**、Maven 发布流程、SemVer 版本策略、体积预算、minSdk 策略、多 ABI、**Crash 隔离、线程管理（命名/守护线程/协程隔离）**、资源隔离、Sample App、Dokka、CHANGELOG 与迁移指南、10 道面试题 | 高 |
| SDK 多产品线变更管理 | `Android/工程化与DevOps/SDK多产品线变更管理.md` | Core + Flavor 分层架构（策略接口+依赖倒置）、Feature Flag 与配置差异化（编译时/运行时选型、生命周期管理）、模块依赖拓扑、**变更影响评估流水线（Metalava API Diff / japicmp / @UsedBy 注解自动影响矩阵）**、行为变更评估（功能/性能/线程/生命周期四类 Checklist）、依赖传导分析、**P0-P3 影响分级与自动通知**、版本策略与分支模型、接口版本化（V1 桥接 V2）、Feature Flag 门控发布、**Breaking Change 六 Sprint 协调流程**、兼容性验证矩阵（Pairwise Testing）、**Multi-Sample CI 三阶段 Pipeline（静态分析→全产品线构建→集成验证）**、契约测试、灰度发布四阶段分级、监控指标与回滚策略、端到端实战案例 + 3 个补充案例、7 道面试题 | 高 |
| WebView 与混合开发 | `Android/UI与渲染/WebView与混合开发.md` | WebView 内核演进（WebKit→Chromium→Trichrome）、初始化与生命周期管理（MutableContextWrapper/独立进程/正确销毁流程）、WebSettings 五种缓存模式与混合内容策略、**WebViewClient/WebChromeClient 核心回调详解（shouldOverrideUrlLoading 调用时机与不调用场景/shouldInterceptRequest 六大业务场景/onPageStarted-Finished-CommitVisible 时序/onReceivedError 新旧 API 差异/onRenderProcessGone）**、**六种 JS-Native 通信方式全景对比（evaluateJavascript/loadUrl/JavascriptInterface/URL Scheme/WebMessagePort/onJsPrompt）**、自建 JSBridge 协议设计（消息格式/callbackId 管理/超时机制）、**性能优化六层策略（startUpWebView/warmUpRendererProcess/预创建复用池/preconnect/prefetchUrl/prerenderUrl）**、离线包方案与增量更新、并行加载首屏加速、Chrome Custom Tabs 三级预加载、安全（RCE 漏洞/SSL 处理/file 协议/XSS 防护/WebViewAssetLoader）、Hybrid 容器化架构与插件机制、Native-H5 职责划分、多进程架构、10 道面试题 | 高 |
| Crash 治理与线上稳定性 | `Android/性能优化/Crash治理与线上稳定性.md` | 稳定性度量维度与业界基准、**Java Crash 捕获机制（UncaughtExceptionHandler 调用链/Handler 链式保存/多 SDK 共存冲突）**、Crash SDK 实现原理（信息采集/mmap 持久化/下次启动上报）、特殊场景（Looper 续命/OOM 预留内存/协程异常双通道）、**Native Crash 捕获（Linux 信号机制/sigaction/sigaltstack 替代栈/async-signal-safe 限制）**、栈回溯（libunwindstack/Frame Pointer）、**堆栈符号化（addr2line/ndk-stack/Breakpad-Crashpad 方案/符号表归档）**、tombstone 文件结构解读、JNI 异常协作、**Crash 分类体系（P0-P3 分级/归因分类）**、聚合指纹提取算法、Top Crash 治理流程、疑难 Crash 定位技巧（堆栈不完整/偶现竞态/ROM 兼容性）、**APM 整体架构（五层分层/采样策略）**、核心监控指标（稳定性/启动/流畅性/网络/资源）、关键采集技术（卡顿检测/网络监控/内存监控）、告警策略（绝对阈值/环比突增/基线对比）、主流方案对比、SDK Crash 隔离三层模型、SDK 版本粒度监控、宿主兼容性 Crash 排查、10 道面试题 | 高 |
